...
export const Todo = () => {
    ...
    const deleteTodo = (id) => {
        axios.delete(`http://localhost:8080/todo/${id}`).then(data => {
            getTodosFetch()
        }).catch(err => console.log(err))
    }
    const editTodo = (id) => {
    ...
    return <>
        <div className='container'>
            <h2 className='text-center fs-1 mt-5'>Todo</h2>

            <form className='mb-5' onSubmit={(evt) => {
                evt.preventDefault();
                hendelSubmit()
            }}>
                <div class="input-group mb-3 mt-4 w-50 mx-auto">
                    <input type="text" ref={TodoInputRef} class="form-control" placeholder="Todo . . ." required/>
                    <button className="btn btn-success" type="submit">Add</button>
                </div>
            </form>

            <ul className='list-unstyled mb-0'>
                {todo.todo.data?.map(item => <TodoItem obj={item} deleteFn={deleteTodo} editFn={editTodo}/>)  }
            </ul>
        </div>
    </>
}
